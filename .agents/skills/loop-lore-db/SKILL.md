...
4. **Enum files** (one per domain):
   ...
   - `enums-core.ts`, `enums-content.ts`, `enums-generation.ts`, `enums-story.ts`, `enums-config.ts`, `enums-character.ts`, `enums-crafting.ts`, `enums-gm.ts`.
...
bun run db:schemas:check                            # gate — fails red if any generated artifact is stale
```

`db:schemas:check` runs as part of `bun run check`; never hand-edit the
