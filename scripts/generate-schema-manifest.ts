...
function parseMigration(filePath: string): {
  ...
  const createTableRegex =
    /\.createTable\(\s*"(\w+)"\s*,?\s*\)[\s\S]*?\.execute\(\)/g;
  let match;
  ...
  const alterBlockRegex =
    /\.alterTable\(\s*"(\w+)"\s*,?\s*\)[\s\S]*?\.execute\(\)/g;
...
function generateManifest(
  ...
  lines.push(`      const actualCols = (sqlite.query(\`PRAGMA table_info("\${name}")\`,).all() as { name: string }[]).map(`);
  lines.push(`        (r,) => r.name,`);
  lines.push(`      );`);
  lines.push(``);
  ...
  for (const [name, cols] of sorted) {
    let group = "Misc";
    if (name.startsWith("user") || name.startsWith("session") || name.startsWith("persona")) group = "Core: Users & Sessions";
    else if (name.startsWith("chat") || name === "chats" || name === "messages") group = "Core: Chats & Messages";
    ...
    else if (name.startsWith("world") || name.startsWith("location") || name === "items" || name.startsWith("quest")) group = "World & RPG";
    else if (name.startsWith("asset")) group = "Assets";
    else if (name.startsWith("model") || name.startsWith("provider") || name.startsWith("prompt") || name.startsWith("template")) group = "Generation & Prompts";
    else if (name.startsWith("plugin") || name.startsWith("event") || name === "event_log") group = "Plugin System";
    else if (name.startsWith("admin") || name.startsWith("config")) group = "Admin & Config";
    else if (name.startsWith("turn") || name.startsWith("story") || name.startsWith("scene")) group = "Turn & Story";
    else if (name.startsWith("encryption") || name.startsWith("key")) group = "Encryption";
    else if (name.startsWith("nsfw") || name.startsWith("content_flag") || name.startsWith("moderation") || name.startsWith("age_gate")) group = "NSFW & Moderation";
    else if (name.startsWith("blog")) group = "Blog System";
    ...
    else if (name.startsWith("bookmark") || name.startsWith("favorite") || name.startsWith("rating") || name.startsWith("history") || name.startsWith("pin") || name === "pins") group = "Bookmarks & History";
...
function main() {
  ...
  const allFiles = [...files.map((f) => join(MIGRATIONS_DIR, f)),
    ...partFiles.map((f) => join(partsDir, f))].sort((a, b) => {
