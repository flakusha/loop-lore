...
describe("Message Variants, Visibility, Status E2E", () => {
  ...
  beforeAll(async () => {
    ...
    const r1 = await api.post<{ id: string }>(`/api/v1/chats/${SEED.chat.id}/messages`, {
  ...
  test("GET /api/v1/messages/:id/variants returns variants array", async () => {
    const res = await api.get<Array<{ content: string }>>(`/api/v1/messages/${variantMsgId}/variants`,);
  ...
  test("PUT /api/v1/messages/:id/visibility updates visibility", async () => {
  ...
  test("PUT /api/v1/messages/:id/visibility requires valid visibility", async () => {
    const res = await api.put(`/api/v1/messages/${SEED.message.id}/visibility`, {
      visibility: "invalid",
    },);
    expect(res.ok,).toBe(false,);
