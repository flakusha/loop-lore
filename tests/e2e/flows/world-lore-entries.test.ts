...
describe("World Lore Entries E2E", () => {
  ...
  test("GET /api/v1/worlds/:id/lore-entries returns empty list", async () => {
    const res = await api.get(`/api/v1/worlds/${worldId}/lore-entries`,);
    expect(res.ok,).toBe(true,);
    const body = res.data as { data?: unknown[] };
    expect(Array.isArray(body.data,),).toBe(true,);
  });

  test("POST /api/v1/worlds/:id/lore-entries creates entry", async () => {
    const res = await api.post<Record<string, unknown>>(`/api/v1/worlds/${worldId}/lore-entries`, {
      key: "world_history",
  ...
  test("GET /api/v1/worlds/:id/lore-entries lists entries", async () => {
    const res = await api.get(`/api/v1/worlds/${worldId}/lore-entries`,);
    expect(res.ok,).toBe(true,);
    const body = res.data as { data?: unknown[] };
  ...
  test("GET /api/v1/worlds/:id/lore-entries/:entryId returns single entry (has keys array, not single key)", async () => {
    const createRes = await api.post<Record<string, unknown>>(`/api/v1/worlds/${worldId}/lore-entries`, {
      key: "specific_lore",
  ...
  test("PUT /api/v1/worlds/:id/lore-entries/:entryId updates entry", async () => {
    const createRes = await api.post<Record<string, unknown>>(`/api/v1/worlds/${worldId}/lore-entries`, {
      key: "update_lore",
      content: "Old content",
    },);
  ...
  test("DELETE /api/v1/worlds/:id/lore-entries/:entryId deletes entry", async () => {
    const createRes = await api.post<Record<string, unknown>>(`/api/v1/worlds/${worldId}/lore-entries`, {
      key: "delete_lore",
      content: "To delete",
    },);
    ...
    const getRes = await api.get(`/api/v1/worlds/${worldId}/lore-entries/${entryId}`,);
