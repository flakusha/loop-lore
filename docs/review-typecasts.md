...
catches when the cast result is passed unsafely. But no rule catches *that a cast
exists* — the grammar of `as` is intentional by the author. `no-unnecessary-type-assertion`
...
| File | Cast | Avoidable? |
|------|------|------------|
| `story/turn-manager.ts:65` | `JSON.parse(chat.story_state) as TurnManagerState` | Not at this level — JSON.parse always returns `any`. A Zod schema on the stored JSON would validate *and* type. |
...
`string` to `GenerationStatus` is a downcast. It *technically* does in strict
