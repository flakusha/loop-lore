...
| **World timeline — backstory seeding (ledger)**                                                                                                                              | 🟡 Partial — §5.2 done (as before). §5.3 done 2026-09-10 (G2): `world_event_steerings` table (folded into `parts/003_worlds.ts` + backfill v26), service `src/story/timeline/event-steering.ts` (`createSteering`/`rollSteering`/`resolveSteering`/`listPendingSteerings`, red-herring guard, manifest appends timeline event). §5.4 service done: `getConvergentEvents` (sibling-story exclusion, newest-first). OPEN: branching UI + `loreSection` foreshadowing/convergence injection |
