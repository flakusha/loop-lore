...
| `src/db/migrations/008_game_interactions.ts` | Append-only `interaction_logs` ledger for resolved game interactions; generated Kysely types live in `src/db/schema-core.ts` |
...
| #   | File                                      | Tables / purpose |
| --- | ----------------------------------------- | ---------------- |
| 001 | `001_init.ts`                             | Core application schema and indexes |
| 002 | `002_shadow_notes_visibility.ts`           | Shadow-note visibility and lifecycle fields |
| 003 | `003_memory_audit_log_action_check.ts`    | Memory audit log and action constraints |
| 004 | `004_shadow_notes_ttl_and_author_type.ts` | Shadow-note TTL and author type |
| 005 | `005_world_lore_lifecycle.ts`             | World-lore lifecycle state |
| 006 | `006_worlds_rules_column.ts`               | World rules configuration |
| 007 | `007_rpg_question_answer_kinds.ts`         | RPG question/answer kind values |
| 008 | `008_game_interactions.ts`                 | Append-only `interaction_logs` ledger with roll math, modifiers, outcomes, and state changes |

`001_init.ts` remains the schema baseline. Migrations 002-007 add focused
columns and tables; migration 008 adds the cross-system interaction ledger.
