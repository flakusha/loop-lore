...
| 🟡 | `tests/e2e/flows/browser/*.test.ts` | Browser E2E flaky: `data-testid` attributes missing in view templates, chrome timeout under load. Pass in isolation; fail under parallel suite. Infrastructure issue, not code bug. |
| 🟡 | `tests/e2e/flows/chat-full.test.ts` | Swipe variants flaky under parallel: `smk.ts` module-level `activeSmk` shared across unit+e2e test files. SMK loaded by crypto unit tests leaks into e2e when files run in parallel. Pass with `--parallel=1`. Root fix: make `activeSmk` per-test-instance rather than singleton. |
