...
| ~~`src/crypto/e2e/key-bundle.ts`~~                    | ~~1 `JSON.parse` + 2 `JSON.stringify`~~ ⬜ Deleted (BUG-private-tier) |
...
| ~~`src/crypto/e2e/key-bundle.ts`~~ | ~~`Buffer.from(b64, "base64")` → `safeFromBase64`~~ ⬜ Deleted (BUG-private-tier)   |
