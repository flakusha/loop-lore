...
- **Quick wins** (#6, #8, #18, #27, #28): promoted to `.plan/backlog/` § Quick Wins and `.plan/epics/epic-architecture.md` § v0.1 MVP.
- **Coverage gaps** (sandbox, RAG, audit, RBAC, cost, DAG, approval gates): tracked as enterprise blockers in `.plan/backlog/` — not yet in any epic. Add to P3 when epics are scoped.
