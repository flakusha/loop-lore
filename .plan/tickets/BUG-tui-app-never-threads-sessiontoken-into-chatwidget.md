...
- `src/config/sections/tui.ts:13-39` — `TuiSection` now carries `sessionToken?: string` (no `TUI_DEFAULTS` entry, so absence = anonymous); constructor normalizes `""` to `undefined`. `tuiMeta.properties.sessionToken` added with the required Bearer-token description.
