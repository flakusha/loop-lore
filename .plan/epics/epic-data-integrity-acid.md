...
| Row versioning    | `data_version` cols on core tables (`users`, `actors`, `messages`, `personas`, …) | ✅    | Optimistic-concurrency anchor — **write path does not enforce it yet**. |
...
1. **`data_version` is dead infrastructure.** Columns exist on most tables but the write path never checks them — concurrent edits to the same row can clobber without detection.
...
### Phase 2 — `data_version` Optimistic Concurrency

**Goal:** Detect and reject stale writes.

| Task                                                                                                       | Files                               | Effort |
| ---------------------------------------------------------------------------------------------------------- | ----------------------------------- | ------ |
| Write policy: `UPDATE … WHERE id=? AND data_version=?` → conflict on 0 rows                                | route handlers + `src/db/*` helpers | Med    |
...
| Write-path notes: where `data_version` applies, transaction boundaries | `docs/spec/deployment.md`       | Low    |
...
**Consequence:** Topologies D/E are Postgres-only. SQLite is correct only for A/B. `data_version` gives row-level optimistic concurrency on either backend once Phase 2 enforces it.

---

## Open Questions

1. **`data_version` scope:** all tables or high-contention first? Recommend high-contention first (Phase 2).
...
3. Phase 2 (`data_version` enforcement) — higher effort, can follow Epic 26.
...
- Present: `data_version` columns, `data_migrations`, WAL init, config loader.
- New: none.
...
- `src/db/schema-*.ts` — `data_version` columns
