...
- **B4:** 5-tier `NSFWContentRating` (`src/schemas/nsfw-rating.ts`) not referenced by `src/chat/types/nsfw.ts`/`moderation.ts`; intensity unwired to enforcement. See TASK-nsfw-rating-enforcement, TASK-nsfw-rating-schema.
- **B7:** `src/chat/types/nsfw.ts` uses raw `Date` — branded/safe-type convention violation. See TASK-nsfw-rating-schema.

_Source: chat functionality audit (loop-lore), 2026-08-25._    ["B7", "RECLASSIFIED 2026-08-25 — NOT a defect. Raw `Date` is idiomatic project-wide; branded timestamps only at serialization boundaries. No change. Real gap is B4 (enum unwired)." The broader need for proper date representation (locale/region + IANA timezone, backend + frontend) is now tracked as FEAT-unified-date-representation-util-locale-region-iana-timezone (issue 7118f58, epic-i18n).`],!-- SPDX-License-Identifier: Apache-2.0 -->
<!-- SPDX-FileCopyrightText: 2026 Loop Lore Contributors -->
