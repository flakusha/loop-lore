...
| Tool calling display          | Persisted `tool_result` rows render as collapsible blocks in chat (`message-list.html`); backend persist helper in `src/generation/generate-route/tool-result-persist.ts`                            | ✅ Partial — persisted tool calls/results visible in chat; richer function-call + params bubble UI open                                         |
