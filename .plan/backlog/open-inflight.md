...
| G2  | World timeline §5.3 forward-event steering + §5.4 cross-story convergence              | `epic-world-timeline*` (cluster B)           | ▲ now                           | 🟡 **Service layer DONE 2026-09-10** (worktree `g2-timeline-steering`) — `world_event_steerings` folded into `parts/003_worlds.ts` + backfill v26; `event-steering.ts` (create/roll/resolve/list, red-herring guard) + `getConvergentEvents`; 12 tests. OPEN: branching UI + `loreSection` injection |
...
- **Mesh sharing landed** — encrypted sharing with reservations and delivery; sender
