...
| 3 | `migrateChat` parent_id not remapped (broken tree)                                          | `src/chat/service/carry-history.ts:50-52`             | 🟡 Bug — copies verbatim; migrated tree points at stale source-chat ids |
| 4 | `chat-search` param validation mismatch                                                      | `src/routes/chat-search/transfer.ts:71`              | 🟡 Bug — route `:id`, handler reads `.id`, schema declares `chatId` → 422 |
