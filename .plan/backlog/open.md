...
| --- | --- | --- | --- | --- |
|| A5 | Lint-ts debt (291 files) → `check` 16/17 | `../priority.md` "Open → close" | ▲ now | 🟡 Still open → `bun run lint` exits 1 (0 errors but 196 warnings; ESLint 10 flat config fails on warnings). Verified 2026-08-12 — do NOT mark done |
|| A6 | Size-strict debt (10 files) → `check` 17/17 | `../priority.md` "Open → close" | ▲ now | ✅ Debt closed → 0 files over 250L (2026-08-12); remaining promotion-to-CI tracked in `TASK-size-strict-debt.md` |
...
| #  | Item                                                                                                                                                              | Where                                                          | Status                                                                                                                                                                                                                     |
| -- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1  | Entire transport module (~9 files) unwired                                                                                                                        | `src/transport/`                                               | 🟡 WIRE → `TASK-transport-server-wiring`: imported only by itself+tests; adapters are no-op stubs (`h2.ts`/`http1.ts` send() = Promise.resolve()); no WS server. Server serves via Bun native HTTP/1.1+TLS (`src/server/start.ts`); SSE handled separately in route modules. Keep, revisit when real-time chat ships |
|| 2  | Telemetry `startRetentionCleanup` never called                                                                                                                    | `src/telemetry/cleanup.ts:12`                                  | ✅ Wired on server startup — `src/server/start.ts:154-155` calls `startRetentionCleanup(database)` (startup + every 24h) |
...
|| 7  | 11 dead chat/service methods reimplemented inline in routes                                                                                                       | `src/chat/service.ts` (now `src/chat/service/`)               | ✅ Superseded — chat service deep-split into `src/chat/service/*` (2026-08-12); `knip` wired as a blocking dead-code gate (`e053e20e`) catches unused exports. Resolved as stale claim |
| 8  | Frontend dead modules: `touch.ts`, `vendor.ts`, `app.ts`; Alpine components (`context-window`, `response-length`, `command-buttons`, `locale-picker`)             | `src/frontend/`                                                | 🟡 WIRE → `TASK-remove-dead-frontend-modules`: `context-window`/`response-length`/`command-buttons` now WIRED (`alpine/index.ts:29-31`); still DEAD: `touch.ts`, root `app.ts`, `vendor.ts` (all superseded by `alpine-init.ts`), `locale-picker.ts` never imported (verified 2026-08-08) — scheduled for deletion |
...
| 12 | RPG services unwired from production (Skills, Quest, Recipes, Achievements, NpcNavigation, Replayability, WorldLocationTraits) + combat/loot/xp no prod consumers | `src/rpg/*/service/`                                           | 🟡 WIRE → `TASK-wire-achievements-routes`, `TASK-wire-skills-routes`, `TASK-wire-quests-routes`, `TASK-wire-npc-navigation-routes`, `TASK-wire-replayability-routes`, `TASK-wire-world-location-traits-routes`, `TASK-wire-xp-loot-routes`, `TASK-wire-combat-routes` (crafting: `TASK-complete-crafting-system-services` + `TASK-wire-crafting-routes`). WIRED: dice, stats, body-systems, encounters, fantasies, intimacy, seduction, `rpg/service.logDiceRoll`. UNWIRED (dead): achievements, replayability, skills, quests (routes/quests uses `story/quest-engine`), npc-navigation, world-location-traits, crafting/recipes, combat, xp, loot + `rpg/service` character-stats/loot/xp exports (verified 2026-08-08) |
...
| 5 | `chat.html` broken Handlebars partial `{{> {{>`                                             | `src/views/chat.html:39-41`                       | ✅ Resolved 2026-08-12 — includes rewritten one-per-line (`chat.html:39-45`); `bun test src/routes/views.test.ts` 65/65 green, no literal `{{>` in served fragment |
| 6 | `content/compress.ts` unguarded `.zst` read (latent crash)                                  | `src/content/compress.ts`                          | ✅ Resolved 2026-08-12 — `.zst` read guarded on `zstdFn` in `compressAssets`; `bun test src/content/` 27/27 green |
|| 7 | `age-gate/controller.ts` error message leak                                                 | `src/age-gate/controller.ts:91,126`               | ✅ Resolved — both handlers return generic `"An error occurred"` + log details server-side (lines 97, 132), verified 2026-08-12 |
|| 8 | Telemetry `chatId` data loss                                                                | `src/frontend/`                                   | ✅ Stale — `trackTelemetry("generation.completed", { chatId })` wired in `chat-generations.ts:33,95`; no concrete loss found (verified 2026-08-12) |

## Release hardening (mirrors `../priority.md` "Open → close"; kept here for the open queue)

- 🟡 **Lint-ts debt** → **open** — `bun run lint` exits 1 (0 errors, ~196 warnings; ESLint 10 flat config fails on warnings). Requires warning remediation (see `TASK-PLAN-LINT-TS-DEBT`).
- ✅ **Size-strict debt** → closed (`size:strict` reports 0 files over 250L in `bun run check`, verified 2026-08-12; gate stays non-blocking until `TASK-size-strict-debt.md` promotion AC lands).
- 🟡 e2e browser auth-loop → `TASK-PLAN-E2E-STABILIZATION`.
- 🟡 release-process + tag `v0.1.0` + changelog → `TASK-PLAN-RELEASE-V010`.
