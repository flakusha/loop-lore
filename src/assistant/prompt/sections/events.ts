...
interface ChatEvent {
  type: "world" | "ambient" | "location" | "random";
  content: string;
  importance: number;
}
...
/**
 * Fetch active persisted random events for the chat (not yet expired).
 *
 * These are generated by `applyPostStoreEffects` (post-store) and stored
 * in `chat_random_events`; surfacing them here makes the next prompt
