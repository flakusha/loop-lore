...
describe("makeAttackRoll", () => {
  ...
  it("applies damage resistance", () => {
    ...
    for (let i = 0; i < 200; i++) {
      const result = makeAttackRoll(
        attacker, target, "str", 2, 6, "fire", 0, resistances,
      );
  ...
  it("applies vulnerability (double damage)", () => {
    ...
    for (let i = 0; i < 200; i++) {
      const result = makeAttackRoll(
        attacker, target, "str", 2, 6, "ice", 0, resistances,
      );
      if (result.hit && result.damage) {
        expect(result.damage.finalDamage).toBe(
  ...
  it("applies immunity (zero damage)", () => {
    ...
    for (let i = 0; i < 200; i++) {
      const result = makeAttackRoll(
        attacker, target, "str", 2, 6, "poison", 0, resistances,
      );
      if (result.hit && result.damage) {
  ...
  it("adds extra flat damage", () => {
    for (let i = 0; i < 200; i++) {
      const result = makeAttackRoll(
        attacker, target, "str", 2, 6, "physical", 5,
      );
      if (result.hit && result.damage) {
