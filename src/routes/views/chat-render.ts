...
> {
  const chatIds = Array.from(chats, (c,) => c.id,);
  const [countsResult, lastMsgsResult,] = await Promise.allSettled([
  ...
  const countMap = new Map<string, number>(Array.from(counts, (r,) => [r.chat_id, Number(r.cnt,),],),);
