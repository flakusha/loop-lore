...
import { MessageRole, MessageContentType, MessageVisibility, MessageStatus } from "../db/enums";
...
interface ListMessagesOpts { database: Kysely<DB>; chatId: string; page: number; pageSize: number; parentId?: string; context: RequestContext; }
interface CreateMessageOpts { database: Kysely<DB>; chatId: string; body: Record<string, unknown>; context: RequestContext; config: Config; }
interface GetMessageOpts { database: Kysely<DB>; messageId: string; context: RequestContext; }
interface ListVariantsOpts { database: Kysely<DB>; messageId: string; context: RequestContext; }
interface SelectVariantOpts { database: Kysely<DB>; messageId: string; body: Record<string, unknown>; context: RequestContext; }
interface DeleteMessageOpts { database: Kysely<DB>; messageId: string; context: RequestContext; }
interface UpdateVisibilityOpts { database: Kysely<DB>; messageId: string; body: Record<string, unknown>; context: RequestContext; }
interface UpdateStatusOpts { database: Kysely<DB>; messageId: string; body: Record<string, unknown>; context: RequestContext; }
...
const dispatch: RouteDispatch = async ({ request, context, database, config }) => {
  ...
  if (singleMatch) {
    ...
    if (method === "GET" && !subRoute) {
      return handleGetMessage({ database, messageId, context });
    }
    if (method === "GET" && subRoute === "/variants") {
      return handleListVariants({ database, messageId, context });
    }
  ...
  if (method === "GET") {
    ...
    return handleListMessages({ database, chatId, page, pageSize, parentId, context });
  }
...
async function enrichAttachments(database: Kysely<DB>, attachmentsJson: string | null): Promise<object | null> {
...
async function handleListMessages(
  { database, chatId, page, pageSize, parentId }: ListMessagesOpts,
): Promise<Response> {
  const offset = (page - 1) * pageSize;

  let query = database.selectFrom("messages").select(database.fn.countAll<number>().as("total")).where("chat_id", "=", chatId);
...
async function handleCreateMessage(
  { database, chatId, body, context, config }: CreateMessageOpts,
): Promise<Response> {
  ...
  // ── Attachments ──────────────────────────────────────────
  const attachments = body.attachments as { assetId: string; order?: number; caption?: string; label?: string }[] | undefined;
  if (attachments && attachments.length > 0) {
    ...
    await database
      ...
      .set({ attachments: (() => { const r = safeJsonStringify(attachData); return r.ok ? r.value : "[]"; })() })
      .where("id", "=", id)
      .execute();
  }
...
async function handleGetMessage(
  { database, messageId, context }: GetMessageOpts,
): Promise<Response> {
  const userId = context.userId;
  if (!userId) return jsonError("Unauthorized", HttpStatus.Unauthorized, ErrorCode.Unauthorized);

  const message = await database.selectFrom("messages").selectAll().where("id", "=", messageId).executeTakeFirst();
...
async function handleListVariants(
  { database, messageId, context }: ListVariantsOpts,
): Promise<Response> {
  const userId = context.userId;
  if (!userId) return jsonError("Unauthorized", HttpStatus.Unauthorized, ErrorCode.Unauthorized);
...
async function handleSelectVariant(
  { database, messageId, body, context }: SelectVariantOpts,
): Promise<Response> {
  const userId = context.userId;
  if (!userId) return jsonError("Unauthorized", HttpStatus.Unauthorized, ErrorCode.Unauthorized);
...
async function handleDeleteMessage(
  { database, messageId, context }: DeleteMessageOpts,
): Promise<Response> {
  const actorId = context.userId;
  if (!actorId) return jsonError("Unauthorized", HttpStatus.Unauthorized, ErrorCode.Unauthorized);
...
async function handleUpdateVisibility(
  { database, messageId, body, context }: UpdateVisibilityOpts,
): Promise<Response> {
  const userId = context.userId;
  if (!userId) return jsonError("Unauthorized", HttpStatus.Unauthorized, ErrorCode.Unauthorized);
  ...
  // Verify ownership: message's chat belongs to user
  const msgChatId = await database.selectFrom("messages").select("chat_id").where("id", "=", messageId).executeTakeFirst();
...
async function handleUpdateStatus(
  { database, messageId, body, context }: UpdateStatusOpts,
): Promise<Response> {
