...
export function tradeRoutes({ database, }: { database: Db },): Elysia {
  ...
  return (
    ...
    .post("/api/worlds/:worldId/trade/execute", async (ctx: any) => {
      ...
      params: t.Object({ worldId: Id, },),
      body: executeBody,
      response: { 200: executeResponse, 400: ErrorResponse, 401: ErrorResponse, 403: ErrorResponse, 404: ErrorResponse, },
      detail: { summary: "Execute trade", description: "Atomically exchange items + gold between two actors.", tags: ["Trade",], },
    },)
