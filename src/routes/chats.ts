  ...
  return new Elysia({ name: "chats" })
    ...
    .post("/api/chats", async (ctx: any) => {
      ...
      if (type && !validTypes.includes(type))
        return jsonError({
          message: `Invalid chat type: ${type}`,
          status: HttpStatus.BadRequest,
          code: ErrorCode.BadRequest,
        });
      if (mode && !validModes.includes(mode))
        return jsonError({
          message: `Invalid chat mode: ${mode}`,
          status: HttpStatus.BadRequest,
          code: ErrorCode.BadRequest,
        });
      if (turnStrategy && !validStrategies.includes(turnStrategy))
        return jsonError({
          message: `Invalid turn strategy: ${turnStrategy}`,
          status: HttpStatus.BadRequest,
          code: ErrorCode.BadRequest,
        });
    ...
    .put("/api/chats/:id", async (ctx: any) => {
      ...
      if (typeof ctx.body.isPinned === "boolean")
        updates.is_pinned = ctx.body.isPinned ? PinnedState.Pinned : PinnedState.Unpinned;
        ...
        .where((eb) =>
          eb.or([
            eb(
              "trigger_message_id",
              "in",
              database.selectFrom("messages").select("id").where("chat_id", "=", ctx.params.id),
            ),
            eb(
              "trigger_turn_id",
              "in",
              database.selectFrom("story_turns").select("id").where("chat_id", "=", ctx.params.id),
            ),
          ]),
        )
        .execute();
    ...
    .post("/api/chats/:id/participants", async (ctx: any) => {
      ...
      try {
        await database
          .insertInto("chat_participants")
          .values({ chat_id: ctx.params.id, actor_id: actorId, role_in_chat: role as ChatParticipantRole })
          .execute();
      } catch {}
    ...
    .put("/api/chats/:id/participants/:actorId", async (ctx: any) => {
      ...
      const updates: Record<string, unknown> = {};
      if (typeof ctx.body.talkativity === "number")
        updates.talkativity = Math.min(10, Math.max(1, ctx.body.talkativity));
    ...
    .put("/api/chats/:id/impersonate", async (ctx: any) => {
      ...
      await database
        .updateTable("chat_participants")
        .set({ impersonate_actor_id: impersonateActorId ?? null })
        .where("chat_id", "=", ctx.params.id)
        .where("actor_id", "=", userId)
        .execute();
      return jsonResponse({ ok: true });
    })
    .put("/api/chats/:id/mark-read", async (ctx: any) => {
      const userId = ctx.userId;
      if (!userId)
        return jsonError({
          message: "Unauthorized",
      ...
      const messageId = ctx.body.messageId as string | undefined;
      if (!messageId)
        return jsonError({
