...
describe("musicLinksRoutes coverage", () => {
  ...
  test("401 across share/list/delete without a userId", async () => {
    ...
    const share = await app.handle(new Request(`http://localhost/api/chats/${chatId}/music-links`, {
      method: "POST",
  ...
  test("POST persists a link and returns the response shape", async () => {
    ...
    const res = await app.handle(new Request(`http://localhost/api/chats/${chatId}/music-links`, {
      method: "POST",
      headers: { "Content-Type": "application/json", },
  ...
  test("POST 422 for missing url, 400 for malformed JSON, 422 for wrong content-type", async () => {
    const app = makeApp(owner, "user",);
    const missing = await app.handle(new Request(`http://localhost/api/chats/${chatId}/music-links`, {
      method: "POST",
      headers: { "Content-Type": "application/json", },
      body: JSON.stringify({},),
    },),);
    expect(missing.status,).toBe(422,);
    const malformed = await app.handle(new Request(`http://localhost/api/chats/${chatId}/music-links`, {
      method: "POST",
      headers: { "Content-Type": "application/json", },
      body: "not-json{{{",
    },),);
    expect(malformed.status,).toBe(400,);
    const textBody = await app.handle(new Request(`http://localhost/api/chats/${chatId}/music-links`, {
      method: "POST",
  ...
  test("POST/GET 404 for unknown chats and chats without access", async () => {
    const app = makeApp(owner, "user",);
    const missing = await app.handle(new Request(`http://localhost/api/chats/${uid()}/music-links`, {
      method: "POST",
      headers: { "Content-Type": "application/json", },
      body: JSON.stringify({ url: "https://open.spotify.com/track/abc123", },),
    },),);
    ...
    const denied = await outsider.handle(new Request(`http://localhost/api/chats/${chatId}/music-links`, {
      method: "POST",
      headers: { "Content-Type": "application/json", },
      body: JSON.stringify({ url: "https://open.spotify.com/track/abc123", },),
    },),);
  ...
  test("DELETE lets the sender remove a link and 404s afterwards", async () => {
    const app = makeApp(owner, "user",);
    const created = await app.handle(new Request(`http://localhost/api/chats/${chatId}/music-links`, {
      method: "POST",
      headers: { "Content-Type": "application/json", },
