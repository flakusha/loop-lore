...
beforeAll(async () => {
  ...
  const userRow = await db.db.selectFrom("users",).select("id",).where("username", "=", "serve-owner",)
    .executeTakeFirstOrThrow();
  ownerId = userRow.id;
  // Persisted asset used by compressed and download tests
  await insertAssets(db.db, ownerId, "serve.png", "image/png", "image" as never, 8, `raw/s1/a2/${ASSET_ID}.png`, {
  ...
  await insertAssets(db.db, ownerId, "deleted.png", "image/png", "image" as never, 8, `raw/d0/00/deleted.png`, {
    id: DELETED_ASSET_ID as never,
    visibility: "public" as never,
  },);
},);
