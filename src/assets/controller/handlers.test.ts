...
beforeAll(async () => {
  ...
  await insertAssets(db, ownerId, "asset.png", "image/png", "image" as never, 8, `raw/e1/a2/${ASSET_ID}.png`, {
    id: ASSET_ID as never,
  },);
  await insertAssets(db, ownerId, "shared.png", "image/png", "image" as never, 8, `raw/e2/a2/${SHARED_ASSET}.png`, {
    id: SHARED_ASSET as never,
...
describe("handlePatchAsset", () => {
  test("returns 401 when no user is present", async () => {
    const res = await handlePatchAsset({
      database: db,
      config,
      ctx: ctx({ id: ASSET_ID, }, { visibility: "public", }, null,),
    },);
  ...
  test("returns 400 for an invalid visibility value", async () => {
    const res = await handlePatchAsset({
      database: db,
      config,
      ctx: ctx({ id: ASSET_ID, }, { visibility: "deleted", },),
    },);
  ...
  test("returns 404 when non-owner tries to patch", async () => {
    const res = await handlePatchAsset({
      database: db,
      config,
      ctx: ctx({ id: ASSET_ID, }, { visibility: "public", }, peerId,),
    },);
    expect(res.status,).toBe(404,);
  });
  ...
  test("owner can change visibility to public", async () => {
    const res = await handlePatchAsset({
      database: db,
      config,
      ctx: ctx({ id: ASSET_ID, }, { visibility: "public", },),
    },);
    expect(res.status,).toBe(200,);
    const body = await res.json();
  ...
  test("owner can change visibility to shared", async () => {
    const res = await handlePatchAsset({
      database: db,
      config,
      ctx: ctx({ id: ASSET_ID, }, { visibility: "shared", },),
    },);
    expect(res.status,).toBe(200,);
    const body = await res.json();
...
describe("handleDeleteAsset", () => {
  ...
  beforeAll(async () => {
    ...
    await insertAssets(db, ownerId, "todel.png", "image/png", "image" as never, 8, `raw/f1/a2/${deletableId}.png`, {
