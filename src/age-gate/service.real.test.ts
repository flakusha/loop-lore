...
import { AgeGateError, getStatus, UnderageError, validateAge, } from "./service";
...
describe("age-gate getStatus", () => {
  ...
  it("returns disabled when enabled false", () => {
    const s = getStatus({ enabled: false, mode: "self-declaration", minimumAge: 18, }, {
  ...
  it("passes with birth date + accepted", () => {
    const s = getStatus({ enabled: true, mode: "self-declaration", minimumAge: 18, }, {
      birth_date: "1990-01-01",
      age_gate_accepted_at: "2024-01-01",
    },);
