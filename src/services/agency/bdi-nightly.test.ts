...
describe("runNightlyReflectionCycle — first-night creation", () => {
  test("no existing plan → insert new with activities", async () => {
    const result = await runNightlyReflectionCycle(db, ["actor1"], {
      ...
      planRecompute: async () => ({ summary: "hunt the dragon", priority: "high", activities: [
    ...
    const rows = raw.query("SELECT * FROM actor_daily_plans WHERE actor_id='actor1'",).all() as Array<Record<string, unknown>>;
...
describe("runNightlyReflectionCycle — same priority → no revision", () => {
  test("recompute matches existing → zero revisions", async () => {
    raw.exec(`INSERT INTO actor_daily_plans(id, actor_id, plan_date, summary, priority, created_at) VALUES ('plan1', 'actor1', '2026-09-25', 'old', 'normal', datetime('now'));`,);
    const result = await runNightlyReflectionCycle(db, ["actor1"], {
      budgetApprove: async () => true,
