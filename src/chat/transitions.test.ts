...
beforeEach(async () => {
  ...
  // Fresh in-memory DB per test: seed ownership fixtures here (beforeAll
  // rows would land in a discarded instance).
...
describe("promoteMessagesToMemories — ownership guard", () => {
  it("rejects when the actor is not a participant of the chat (memory poisoning defense)", async () => {
    ...
    // `participantIds` claiming victim membership, but the actorId is a
