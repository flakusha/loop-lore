...
import { generateRandomEvent, } from "./random-events";
 ...
 * 4: environmental (weight 1) - "The ground vibrates..."
...
describe("generateRandomEvent", () => {
  ...
  test("uses currentLocation name for {location} when provided", () => {
    // Note: No event in pool has {location} placeholder, but we test the fallback
    let callCount = 0;
    const origRandom = Math.random;
    Math.random = () => {
      callCount++;
    ...
    try {
      ...
      expect(result,).not.toBeNull();
      // Weather event selected - no {location} in template, just verify it works
