...
describe("generateLoot", () => {
  ...
  test("under-leveled monsters drop nothing (level gate)", () => {
    const table: LootTableEntry[] = [{ itemId: "gem", dropChance: 100, minQuantity: 1, maxQuantity: 5, requiredLevel: 5, },];
    expect(generateLoot(table, 1,),).toEqual([],);
  });

  test("zero-percent drops never land", () => {
    const table: LootTableEntry[] = [{ itemId: "gem", dropChance: 0, minQuantity: 1, maxQuantity: 1, requiredLevel: 1, },];
    expect(generateLoot(table, 10,),).toEqual([],);
  });

  test("guaranteed drops land with fixed quantity when min equals max", () => {
    const table: LootTableEntry[] = [{ itemId: "gem", dropChance: 100, minQuantity: 3, maxQuantity: 3, requiredLevel: 1, },];
    expect(generateLoot(table, 10,),).toEqual([{ itemId: "gem", quantity: 3, },]);
  });

  test("guaranteed range drops stay within bounds", () => {
    const table: LootTableEntry[] = [{ itemId: "gem", dropChance: 100, minQuantity: 1, maxQuantity: 6, requiredLevel: 1, },];
    for (let i = 0; i < 20; i++) {
  ...
  test("boundary — monster at exactly the required level may drop", () => {
    const table: LootTableEntry[] = [{ itemId: "gem", dropChance: 100, minQuantity: 1, maxQuantity: 1, requiredLevel: 5, },];
