...
describe("TravelTickEngine", () => {
  ...
  test("tick advances a transport along its route and wraps on loop=1", async () => {
    ...
    const row = testDb.sqlite.query(`SELECT travel_progress FROM locations WHERE id = ?`,).get(ship,) as { travel_progress: number };
  ...
  test("tick pins arrival at the last stop when loop=0", async () => {
    ...
    const engine = new TravelTickEngine(testDb.db,);
    const worldId = await makeWorld();
    const routeId = await routes.createRoute({ worldId, name: "OneWay", kind: "road", loop: false, secondsPerUnit: 60, },);
  ...
  test("tick skips non-moving (anchored) transports", async () => {
    ...
    const engine = new TravelTickEngine(testDb.db,);
    const worldId = await makeWorld();
    const routeId = await routes.createRoute({ worldId, name: "Coast2", kind: "sea", loop: true, secondsPerUnit: 60, },);
    ...
    await routes.attachTransport(ship, routeId,);
    const before = (testDb.sqlite.query(`SELECT travel_progress FROM locations WHERE id = ?`,).get(ship,) as { travel_progress: number }).travel_progress;
    const summary = await engine.tick({ elapsedSeconds: 60, },);
    const after = (testDb.sqlite.query(`SELECT travel_progress FROM locations WHERE id = ?`,).get(ship,) as { travel_progress: number }).travel_progress;
  ...
  test("tick returns summary counts", async () => {
    ...
    const engine = new TravelTickEngine(testDb.db,);
    const worldId = await makeWorld();
    const routeId = await routes.createRoute({ worldId, name: "Coast3", kind: "sea", loop: true, secondsPerUnit: 30, },);
