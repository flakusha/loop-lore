...
async function makeTransportKindLocation(worldId: string, parentId: string | null, mobilityMode: string,): Promise<string> {
  const id = randomUUID();
  testDb.sqlite.run(
...
describe("TravelRouteService", () => {
  ...
  test("attachTransport sets current_route_id and rejects non-transport / static", async () => {
    ...
    const row = testDb.sqlite.query(`SELECT current_route_id, travel_progress FROM locations WHERE id = ?`,).get(ship,) as any;
