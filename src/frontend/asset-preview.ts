...
async function ensurePreviewModal(): Promise<HTMLElement | null> {
  ...
  // First-party, same-origin partial (static modal shell — no user data is
  // interpolated server-side; dynamic fields are set below via textContent /
  // escapeHtml). Sanitizing here would strip the Alpine directives the modal
  // needs, so raw injection is intentional and safe.
