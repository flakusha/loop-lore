...
describe("storyState", () => {
  describe("derived state", () => {
    ...
    test("turnForMessage looks up by parent message id, null when absent", () => {
      ...
      expect(s.turnForMessage("msg-1",),).toEqual({
        turnNumber: 3, qualityScore: 88, promptSent: "go", status: "completed",
      },);
  ...
  describe("_loadTurns", () => {
    test("indexes turns by parent message id and derives latest-turn state", async () => {
      ...
      mockFetch(200, {
        data: [
          { id: "t1", turn_number: 1, parent_message_id: "msg-1", actor_id: "a1",
            prompt_sent: "GM: enter the cave", quality_score: 92, status: "completed",
            quest_progress: "[]", },
          { id: "t2", turn_number: 2, parent_message_id: "msg-2", actor_id: "a2",
            prompt_sent: "", quality_score: null, status: "pending", quest_progress: "[]", },
        ],
      },);
      await s._loadTurns();
      expect(s.turnMeta["msg-1"],).toEqual({
        turnNumber: 1, qualityScore: 92, promptSent: "GM: enter the cave", status: "completed",
      },);
  ...
  describe("_loadWorldState", () => {
    test("parses location state + NPCs, tolerating shape drift", async () => {
      ...
      fetchHandler = (_url, _opts,) => {
        call++;
        if (call === 1) {
          return Response.json({ id: "loc-1", state: {
            time_of_day: "night", weather: "rain", atmosphere: "tense", description_override: "dark cave",
          }, }, { status: 200, },);
        }
  ...
  describe("controls", () => {
    ...
    test("togglePause notifies instead of toggling on failure", async () => {
      ...
      s.running = true;
      const toasts: string[] = [];
      s.notify = (message: string,) => { toasts.push(message,); };
    ...
    test("createQuest POSTs and reloads quests", async () => {
      ...
      fetchHandler = (_url, _opts,) => {
        ...
        return Response.json({ data: [{ id: "q-new", name: "Side quest", type: "composite",
          status: "active", progress: 0, },], }, { status: 200, },);
      };
  ...
  describe("refresh", () => {
    ...
    test("skips story loads when the chat is not in story mode", async () => {
      ...
      fetchHandler = (_url, _opts,) => {
        call++;
        if (call === 1) { return Response.json({ mode: "direct", world_id: null, gm_config: "{}", }, { status: 200, },); }
    ...
    test("loads all story resources for a story chat", async () => {
      ...
      fetchHandler = (_url, _opts,) => {
        ...
        switch (call) {
          case 1:
            return Response.json({ mode: "story", world_id: "world-1", current_location_id: "loc-1",
              gm_config: "{}", }, { status: 200, },);
          case 2:
