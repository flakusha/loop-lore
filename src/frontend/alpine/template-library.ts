  ...
  return {
    ...
    async startEdit(id: string,) {
      try {
        ...
        const payloadResult = safeJsonStringify(t.payload, 2,);
    ...
    async save() {
      ...
      this.error = "";
      try {
        const payloadResult = safeJsonParse<unknown>(this.draft.payload,);
        if (!payloadResult.ok) {
    ...
    async importPack(file: File,) {
      this.error = "";
      try {
        const packResult = safeJsonParse<unknown>(await file.text(),);
        if (!packResult.ok) {
