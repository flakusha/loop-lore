...
export async function convertPersonaToCharacter(
  ...
  // Wrap the SELECT and INSERT in a single transaction so a concurrent
  // DELETE of the persona between the two statements cannot leave the
  // caller with an orphaned actor row pointing at a vanished persona,
  // and so any insert failure rolls back the implicit existence check
  // without leaking a partial conversion.
