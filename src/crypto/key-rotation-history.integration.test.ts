...
describe("rotateActorKeyAndReEncrypt — history preservation", () => {
  test("5 messages encrypted with OLD chat key survive actor-key rotation and decrypt with NEW chat key", async () => {
    ...
    // ── 1. Snapshot OLD chat key + OLD actor-key id for PARTICIPANT_1 ──────
    const oldChatKey = await deriveChatKeyForChat(db, CHAT_ID, smk);
    ...
    // ── 4. Derive NEW chat key ────────────────────────────────────────────
    const newChatKey = await deriveChatKeyForChat(db, CHAT_ID, smk);
  ...
  test("rotation preserves history across multiple rounds", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      const content = await compressThenEncrypt({ plaintext: `round-1-msg-${i}`, chatKey: oldChatKey.key, keyId: initialKeyId });
      await db.insertInto("messages").values({
        id: `r1-msg-${i}`,
        chat_id: CHAT_ID,
        actor_id: PARTICIPANT_1,
    ...
    const midChatKey = await deriveChatKeyForChat(db, CHAT_ID, smk);

    // Verify round-1 messages decrypt with the post-r1 chat key
    for (let i = 0; i < 3; i++) {
      const row = await db.selectFrom("messages").select("content").where("id", "=", `r1-msg-${i}`).executeTakeFirst();
    ...
    for (let i = 0; i < 2; i++) {
      const content = await compressThenEncrypt({ plaintext: `round-2-msg-${i}`, chatKey: midChatKey.key, keyId: r1.newKeyId });
      await db.insertInto("messages").values({
        id: `r2-msg-${i}`,
        chat_id: CHAT_ID,
        actor_id: PARTICIPANT_1,
    ...
    const finalChatKey = await deriveChatKeyForChat(db, CHAT_ID, smk);
    for (let i = 0; i < 3; i++) {
      const row = await db.selectFrom("messages").select("content").where("id", "=", `r1-msg-${i}`).executeTakeFirst();
    ...
    for (let i = 0; i < 2; i++) {
      const row = await db.selectFrom("messages").select("content").where("id", "=", `r2-msg-${i}`).executeTakeFirst();
      const plaintext = await decryptThenDecompress(row!.content, finalChatKey.key);
