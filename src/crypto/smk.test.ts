...
describe("initSmk — valid key", () => {
  test("64-char hex key loads SMK and enables encryption", async () => {
    await initSmk({ serverEncryptionKey: VALID_64_HEX, required: false });
    const smk = getSmk();
  ...
  test("hex key with dashes and spaces is accepted (cleaned internally)", async () => {
    ...
    await initSmk({ serverEncryptionKey: dashedKey, required: false });
    expect(getSmk()).not.toBeNull();
  });

  test("uppercase hex key works", async () => {
    await initSmk({ serverEncryptionKey: VALID_64_HEX.toUpperCase(), required: false });
    expect(getSmk()).not.toBeNull();
  });
});
...
describe("initSmk — null / dev mode", () => {
  ...
  test("empty string key + required=false returns null", async () => {
    await initSmk({ serverEncryptionKey: "", required: false });
    expect(getSmk()).toBeNull();
  });
...
describe("initSmk — error paths", () => {
  test("missing key with required=true throws", async () => {
    await expect(initSmk({ required: true })).rejects.toThrow("SERVER_ENCRYPTION_KEY is required");
  });

  test("empty key with required=true throws", async () => {
    await expect(initSmk({ serverEncryptionKey: "", required: true })).rejects.toThrow(
      "SERVER_ENCRYPTION_KEY is required",
    );
  });

  test("invalid hex chars throw", async () => {
    await expect(initSmk({ serverEncryptionKey: "gg".repeat(32), required: false })).rejects.toThrow(
      "Invalid hex byte",
    );
  });

  test("odd-length hex throws", async () => {
    await expect(initSmk({ serverEncryptionKey: "a".repeat(63), required: false })).rejects.toThrow();
  });

  test("short key (16 chars, 8 bytes) throws length error", async () => {
    await expect(initSmk({ serverEncryptionKey: "a".repeat(16), required: false })).rejects.toThrow(
      "must be 64 hex chars",
    );
  });

  test("long key (66 chars, 33 bytes) throws length error", async () => {
    await expect(initSmk({ serverEncryptionKey: "a".repeat(66), required: false })).rejects.toThrow(
      "must be 64 hex chars",
    );
  });
});
