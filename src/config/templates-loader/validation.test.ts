...
describe("validateAvatarConfig", () => {
  ...
  test("rejects malformed emotions", () => {
    ...
    expect(() =>
      validateAvatarConfig({ emotions: { happy: { asset: "h.png", intent: 3, }, }, },),
    ).toThrow(/emotions\.happy\.intent must be a string/,);
  });
...
describe("validateImageEditConfig", () => {
  ...
  test("rejects malformed workflows", () => {
    ...
    expect(() =>
      validateImageEditConfig({ workflows: { w: { name: 1, }, }, },),
    ).toThrow(/workflows\.w\.name must be a string/,);
  });
});
...
describe("warnUnknownPromptPurposes", () => {
  ...
  test("stays silent for far-away custom keys", () => {
    const warnings = captureWarnings(() =>
      warnUnknownPromptPurposes({ my_totally_custom_setting: "x", },),
    );
    expect(warnings,).toEqual([],);
  });
